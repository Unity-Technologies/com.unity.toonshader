...
//    float3 _NormalMap_var = UnpackNormalScale(tex2D(_NormalMap, TRANSFORM_TEX(Set_UV0, _NormalMap)), _BumpScale);
 ...
 //   float3 i_normalDir = surfaceData.normalWS;
    ...
    float3 lightColor = max(0, lerp(addPassLightColor, lerp(0, min(addPassLightColor, addPassLightColor / pureIntencity), notDirectional), max(_Is_Filter_LightColor, _ToonHighCutFilter)));
